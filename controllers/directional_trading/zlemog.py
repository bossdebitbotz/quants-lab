...
class ZLEMAConfig(DirectionalTradingControllerConfigBase):
    ...
    interval: str = Field(
        ...
        client_data=ClientFieldData(
            ...
            prompt_on_new=False))
    higher_interval: str = Field(
        default="15m",
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the higher timeframe interval for validation (e.g., 15m, 1h, 4h): ",
            prompt_on_new=True))
    source: str = Field(
        default="hlc3",
        client_data=ClientFieldData(
...
class ZLEMAController(DirectionalTradingControllerBase):
    ...
    def __init__(self, config: ZLEMAConfig, *args, **kwargs):
        ...
        if len(self.config.candles_config) == 0:
            self.config.candles_config = [
                ...
                # Always add higher timeframe candles config
                CandlesConfig(
                    connector=config.candles_connector,
                    trading_pair=config.candles_trading_pair,
                    interval=config.higher_interval,
                    max_records=self.max_records
                )
            ]
    ...
    def kalman_filter(self, src: pd.Series) -> pd.Series:
        ...
        # Initial values - more appropriate defaults
        value1 = 0.0  # Initial price change (neutral starting point)
        value2 = 0.0  # Initial volatility estimate
        value3 = 0.0  # Initial filtered price
    ...
    def get_higher_timeframe_data(self):
        """
        Retrieves the higher timeframe candles data
        """
        try:
            return self.market_data_provider.get_candles_df(
                connector_name=self.config.candles_connector,
                trading_pair=self.config.candles_trading_pair,
                interval=self.config.higher_interval,
                max_records=self.max_records
            )
        except Exception as e:
            self.logger().error(f"Error getting higher timeframe data: {e}")
            return None

    def validate_signal_with_higher_timeframe(self, df, signal):
        """
        Validates the current timeframe signal with a higher timeframe trend direction
        """
        # Get higher timeframe data
        higher_tf = self.get_higher_timeframe_data()
        if higher_tf is None:
            return signal  # Fall back to original signal if higher timeframe data not available
            
        # Calculate ZLEMA on higher timeframe
        src_higher = self.get_price_source(higher_tf)
        src_higher = self.kalman_filter(src_higher)  # Apply Kalman filter to higher timeframe
        ma1_higher = self.zlema(src_higher, self.config.period_fast)
        ma2_higher = self.zlema(src_higher, self.config.period_medium)
        
        # Check if higher timeframe trend aligns with signal
        higher_tf_bullish = ma1_higher.iloc[-1] > ma2_higher.iloc[-1]
        higher_tf_bearish = ma1_higher.iloc[-1] < ma2_higher.iloc[-1]
        
        # Only accept signals that align with higher timeframe trend
        if signal > 0 and not higher_tf_bullish:
            return 0
        if signal < 0 and not higher_tf_bearish:
            return 0
    ...
    async def update_processed_data(self):
        ...
        # Get current signal
        current_signal = df["signal"].iloc[-1] if not df["signal"].empty else 0
        
        # Always validate signal with higher timeframe (mandatory)
        validated_signal = self.validate_signal_with_higher_timeframe(df, current_signal)
        # Store both original and validated signals
        df['validated_signal'] = df['signal'].copy()
        df.loc[df.index[-1], 'validated_signal'] = validated_signal
        
        # Update processed data
        self.processed_data["signal"] = validated_signal
