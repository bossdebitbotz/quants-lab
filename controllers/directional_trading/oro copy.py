...
class OroControllerConfig(DirectionalTradingControllerConfigBase):
    controller_name = "oro"
...
class OroController(DirectionalTradingControllerBase):
    """
    Oro Strategy - Simplified Version
    High-Precision Breakout Detection with ZLEMA Confirmation
    Optimized for production execution
    """

    def __init__(self, config: OroControllerConfig, *args, **kwargs):
        self.config = config
    ...
    async def update_processed_data(self):
        try:
            ...
            try:
                ...
                # Simplified conditions without momentum elements
                long_condition = (
                    (close > df["high_band"].shift(1)) &  # Breakout above previous high
                    (df['ZLEMA_fast'] > df['ZLEMA_medium']) &  # Fast ZLEMA above medium
                    (df['ZLEMA_medium'] > df['ZLEMA_slow']) &  # Medium ZLEMA above slow
                    (df["volume_z_score"] > 1.0) &  # Volume surge
                    (df["strong_trend"])  # Strong trend
                )
                
                short_condition = (
                    (close < df["low_band"].shift(1)) &  # Breakout below previous low
                    (df['ZLEMA_fast'] < df['ZLEMA_medium']) &  # Fast ZLEMA below medium
                    (df['ZLEMA_medium'] < df['ZLEMA_slow']) &  # Medium ZLEMA below slow
                    (df["volume_z_score"] > 1.0) &  # Volume surge
                    (df["strong_trend"])  # Strong trend
                )
