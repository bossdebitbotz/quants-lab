...
from controllers.directional_trading.elitesmugplugobi import EliteSmugPlugObiConfig
from core.backtesting.optimizer import BacktestingConfig, BaseStrategyConfigGenerator


class EliteSmugPlugObiConfigGenerator(BaseStrategyConfigGenerator):
    ...
    async def generate_config(self, trial) -> BacktestingConfig:
        # Generate specific EliteSmugPlugObi metrics
        ...
        # OBI specific parameters
        obi_window = trial.suggest_int("obi_window", 15, 25)  # Centered around 20
        obi_threshold = trial.suggest_float("obi_threshold", 0.5, 0.7, step=0.05)  # Centered around 0.6
        bid_ask_ratio_threshold = trial.suggest_float("bid_ask_ratio_threshold", 1.1, 1.3, step=0.05)  # Centered around 1.2
        ...
        # Id Generation
        controller_id = (f"elite_smugplug_obi_{self.connector_name}_{interval}_{self.trading_pair}_"
                       f"ema_{ema_fast}_{ema_medium}_{ema_long}_"
                       f"atr_{atr_length}_{atr_multiplier}_"
                       f"vol_{volume_surge}_rsi_{rsi_period}_"
                       f"obi_{obi_window}_{obi_threshold}_{bid_ask_ratio_threshold}_"
        ...
        config = EliteSmugPlugObiConfig(
            ...
            obi_threshold=Decimal(obi_threshold),
            bid_ask_ratio_threshold=Decimal(bid_ask_ratio_threshold),
