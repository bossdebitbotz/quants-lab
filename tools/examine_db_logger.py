...
async def examine_db_logger():
    ...
    try:
        ...
        try:
            ...
            # 7. Check trade_decisions table
            print("\n--- Trade Decisions Table Check ---")
            try:
                cursor.execute("SELECT COUNT(*) FROM trade_decisions")
                count = cursor.fetchone()[0]
                print(f"Total trade decisions in database: {count}")
                ...
                if count > 0:
                    ...
                    """)
                    columns = cursor.fetchall()
                    print("Trade decisions table columns:")
                    for col in columns:
                        print(f"  {col[0]}")
                        
                    # Try to get the column with timestamp info (could be 'timestamp' or another name)
                    timestamp_col = None
                    for col in columns:
                    ...
                    if timestamp_col:
                        ...
                        print(f"\nRecent trade decisions (ordered by {timestamp_col}):")
                        for decision in decisions:
                            print(f"  Decision record: {decision}")
                    else:
                        print("Could not identify timestamp column in trade_decisions table")
                else:
                    print("No trade decisions found in the database")
            except Exception as e:
                print(f"Error accessing trade_decisions table: {str(e)}")
